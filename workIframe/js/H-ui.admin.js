	...
	$(".Hui-aside").on("click",".menu_dropdown a",function(){
		if($(this).attr('_href')){
			...
			if(!bStop){
				...
				//iframe_box.find(".show_iframe").hide().eq(bStopIndex).show();//多次点击左侧菜单时，页面不刷新，只激活相应的页面
			}
		}
	});
...
/*
	...
	callback  按钮回调
*/
function layer_show_edit(title,url,w,h,btn_arr,callback){
	if (title == null || title == '') {
		title=false;
	};
	if (url == null || url == '') {
	...
	layer.open({
		...
		yes:function(index, layero){
			if(typeof callback == 'function')callback(index, layero,1);
		},
		cancle:function(index,layero){
			if(typeof callback == 'function')callback(index,layero,2);
			return false;
		},
		btn3:function(index, layero){
			if(typeof callback == 'function')callback(index, layero,3);
			return false;
		}
	});
}
